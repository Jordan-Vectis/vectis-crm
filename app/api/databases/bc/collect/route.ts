...
import { writeBcSale, type FeedLot } from "@/lib/archive-site"
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ok: true, files: files.length, sales, lots,
      ...
      message: lots === 0
        ...
        : `Loaded ${lots.toLocaleString()} lot${lots === 1 ? "" : "s"} across ${sales.toLocaleString()} sale${sales === 1 ? "" : "s"}. Press Copy photos to bring their pictures in.`,
    })
  } catch (e: any) {
