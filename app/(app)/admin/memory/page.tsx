...
- FORGIVING (Jordan: "if I add a , anywhere it doesn't find that one lot"; "Kämmer" found nothing because the lot says "Kammer"). All in lib/search-words.ts: punctuation stripped and little words (&, and, the) dropped by tokens(); plurals via pluralVariants(); accents either way - the typed word is searched as typed AND folded (Kämmer -> kammer), and the spelling list gives the accented spellings our descriptions use (marklin -> märklin); typos via the spelling list.
- NEVER fold or rewrite the descriptions at search time: measured on production, translate() over ArchiveLot took 36 s for "halo" against 3.3 s plain, and every search timed out on staging. Matching stays a plain ILIKE; anything cleverer is an extra spelling of the TYPED word.
- The spelling list: tables SearchWord (folded word, count, accented "forms", pg_trgm GIN index) + SearchWordState + SearchWordBuild (UNLOGGED scratch). Built in the background in 5,000-row batches (~2-3 min, never one long query) the first time it's needed, when BUILD_VERSION changes, and weekly in London evenings (kickWordListBuild), never waited for. A heartbeat on buildingSince lets a later search restart a build a deploy killed. Candidates by trigram, confirmed by edit distance (catches Stieff/Steiff). Only unknown or rare words are corrected, and the page says "Also searched for X (you typed Y)".
...
📚 **Lot Archive (2026-09-07).** /databases/archive — every lot sold before Business Central, searchable (description · sale title · year · hammer range, 100 a page). Tables ArchiveLot (unique on lotId — sale + lot number REPEATS in multi-day sales, so it is only an index), ArchiveImport, ArchiveSale, ArchiveJob. NEEDS Run Migrations. ⚠ The Crystal report is the trusted source, not lot export.xlsx (its 1.5M rows = 946k real + 548k duplicates); the report's own 7,477 blank placeholder lots and 849 unlotted entries are rejected as unreadable on import — expected.

**Two sources, one table.** (1) The old system's export (136 MB xlsx: AuctionID · AuctionDate as US " MM/DD/YYYY" · OnlineTitle · Lot "2,265" · Description · BottomPrice · TopPrice · HammerPrice). ⚠⚠ STREAMED from R2 row by row (readArchiveStream in lib/archive-import.ts) by a server-side loop the page polls — reading it whole with SheetJS killed the request. Resumable; createMany skipDuplicates is the dedupe; .xls must be saved as .xlsx. (2) The website (lib/archive-site.ts, job "site"): walks vectis.co.uk's own sale ids; a sale URL is /bidding/{AuctionID}-{slug}-{siteSaleId} — the FIRST number is the sheet's AuctionID (724 → site 683). Lots come from the site's JSON feed (task=commission.getLots): lot_number, unique_id = the OLD SYSTEM'S LotID (e.g. 764731), image = lot_images/large/{LotID}/{LotID}.webp, description, estimates, hammer, isFinished. Only finished sales are written; existing rows get siteLotId/siteLink/sitePhoto/siteHammerPrice (sheet figures KEPT, blanks filled). ⚠⚠ ANNOTATE ONLY — nothing is created or overwritten from the site (Jordan: "the website has errors"); lots only on the site are just counted. The page links each matched lot to vectis.co.uk via siteLink. Export & handover: admin ⬇ Export data (CSV) streams the whole table (GET /api/databases/archive/export, 5,000 rows a batch, every column incl. LotID + both photo file names + SiteLink); the page documents the R2 naming (archive-photos/xl/{LotID}.webp full-size, archive-photos/{LotID}.webp display) and the rclone bucket-to-bucket handover. Zips-of-everything deliberately NOT built. Job "photos" copies each main photo (~23 KB) into R2 archive-photos/{lotId}.webp.
...
THE FAMILY'S REAL NAME IS "Vectis JO" / "Vectis Jo" (production AiPreset keys, all starred): Dolls and Bears, Model Kits and Aviation, Tv and Film, Comics, Diecast and Tinplate. "Vectis Jo style" means this family.
SPORTS, COLLECTABLES & VINTAGE TOYS ("Vectis Jo: Sports, Collectables & Vintage Toys", 2026-09-10): ONE instruction for the three Hub categories, decide-then-apply-the-section, ~6,500 chars with 8 worked examples. ⚠ Sports lots are mostly catalogued with NO key points (9 of 255 had any) - the cataloguer types the description, and the batch route then sends "Existing description: ... improve and enhance" - so the instruction says the facts come from the key points OR the existing description. ⚠⚠ SIGNATURES AND AUTHENTICITY block is the rule that matters most: never hand-signed, genuine, authenticated, match-worn or player-issue unless recorded in those terms; a signatory only as recorded; a certificate named with its issuer as written and never upgraded to "authenticated"; "no provenance / no COA / no verification / replica / some may be reproductions" said plainly. New to the family: SHIPPING AND NOTICES copied word for word on their own line before the estimate ("CAN ONLY BE SHIPPED WITHIN THE UK" on cap guns, "COLLECTION ONLY", "18+", the NB notice on historical items). Programme runs and card sets go as counts or ranges, never enumerated - the Comics recitation lesson applied up front. "Unchecked for completeness or correctness", "untested", "approximately" and "possible duplication" are kept verbatim.
...
- **Comparables** = our OWN archive, the bit Google Lens can't do. **Since 2026-09-10** (Jordan: "use this new search and database to improve the lens as that is supposed to check our own lot") Lens calls \`findComparables(id, { everywhere: true })\` in \`lib/comparables.ts\`: BC sales with the website's FULL descriptions (not BC's 250-char short ones) AND the ABC archive (948k lots, 1999–2023), with photos, sale, vectis.co.uk links, Website Search's forgiving spellings (accents, plurals, Tri-ang/triang) and the maker narrowed on its distinctive word ("Dinky Toys" → dinky). One query per source with catalogue-number matches sorted BEFORE the cap (trap 2, handled in one scan), both in parallel, 15 s cap each; one source failing keeps the other. Headline = middle half of single-lot sales + typical (median) via \`summariseComparables\`, taken from the lots with the SAME catalogue number when at least two exist ("Same No." chip) — measured: Hornby R3514's neighbours made ~£100, the R3514 itself £190–£300. \`GROUPED\` widened the same day from lots Lens surfaced: "(1) … (2)" lists, "pair of", "trio of", "Locos comprising", "plus others" ("Four 3029 locos Plus others" at £35 had led Märklin 3029), and a quantity ("5 x assorted", "dolls x five") — ⚠ but a quantity ONLY in the first 70 characters: further in it is one product's contents ("R3514 5-Car Train Pack containing … 3 x Passenger Coaches" is ONE item; matching it anywhere set aside every R3514 sale, measured). This also sharpens Valuations and Lotting Up, which share it; each lot shows photo, BC/ABC badge and opens on vectis.co.uk; **"See every match in Website Search"** fires a \`hub:website-search\` window event the Website Search panel listens for (Lens closes WITHOUT resetting, so reopening shows the result). ⚠ Valuations and Lotting Up stay on the quick BC-only default — they call it once per item in a list. Both reaches share ONE \`rank()\`.
