...
- FORGIVING (Jordan: "if I add a , anywhere it doesn't find that one lot"; "Kämmer" found nothing because the lot says "Kammer"). All in lib/search-words.ts: punctuation stripped and little words (&, and, the) dropped by tokenise(); accents folded on BOTH sides - translate() in SQL and foldText() in JS with the SAME map (FOLD_FROM/FOLD_TO; change one, change both); plurals via pluralVariants(); typos via a spelling list.
- The spelling list: tables SearchWord (every word in our descriptions seen twice or more, with a count, pg_trgm GIN index) + SearchWordState. Built in the background the first time it's needed, rebuilt weekly in London evenings (kickWordListBuild), never waited for. Candidates by trigram, confirmed by edit distance (catches Stieff/Steiff). Only unknown or rare words are corrected, and the page says "Also searched for X (you typed Y)".
...
- SPEED, measured on production: gluing description + IDs + sale name per row took ~6 s; description-only matching with the counts as window totals in ONE query is ~2.4 s, even for "corgi" (115k matches). The counts use count(*) FILTER (WHERE source = ...) OVER () - PARTITION BY lost the count of any source with no row on the page. Each source folds its description once per row in an OFFSET 0 subquery.
