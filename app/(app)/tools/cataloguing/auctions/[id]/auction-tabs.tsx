...
function ManageLotsTab({ lots, auctionId, auction, allAuctions, bcLocked, onEdit, onDelete, onTransfer }: {
  ...
  function applyVendorChange() {
    ...
    // A receipt spans many totes, so the server clears the tote rather than guess.
    // Say so — a silently emptied field is how people stop trusting a tool.
    const toteNote = newTote ? "" : "\n\nThe tote is cleared — a receipt covers several totes, and the one on the lot belongs to where it used to be."
  ...
  return (
    <div>
      ...
      {/* ── Set reserve on the ticked lots ──
          ⚠ There is deliberately NO reserve column in the lots table. One was built and Jordan
          took it out: "it does not need to be on the actual table like this, just inside when
          you click on it is fine" (2026-09-02). A single lot is edited in the lot editor, which
          has had a Reserve field all along; this panel is only for putting the same figure on
          several at once. Don't put the column back. */}
      {showReserve && (
