...
function Autocomplete({ value, onChange, options, placeholder, accentColor = CAT_ACCENT }: {
  value: string
  ...
  function select(opt: string) { onChange(opt); setOpen(false) }
  return (
    <div className="relative">
      <div className="flex">
        <input value={value} onChange={e => { onChange(e.target.value); setOpen(true) }}
          ...
          style={{ borderColor: value ? accentColor + "66" : undefined }} />
...
export default function CataloguingPage() {
  ...
  function saveLot() {
    ...
    const lot: CatLot = {
      auctionNo, auctionName, vendor, tote, receipt, barcode, keyPoints,
      mainCat, subCat, brand,
      estLow: estLow.replace(/[£,]/g,"").trim(),
      estHigh: estHigh.replace(/[£,]/g,"").trim(),
      condition: condArr.join(" to "), parcel,
  ...
  function exportExcel() {
    const headers = ["Auction No","Auction Name","Vendor","Tote","Receipt No","Internal Barcode","Key Points","Main Category","Sub Category","Brand","Estimate Low","Estimate High","Condition","Parcel Size","Timestamp"]
    const rows = lots.map(l => [l.auctionNo,l.auctionName,l.vendor,l.tote,l.receipt,l.barcode,l.keyPoints,l.mainCat,l.subCat,l.brand,l.estLow,l.estHigh,l.condition,l.parcel,l.timestamp])
  ...
  function CondBtn({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
    return (
      <button type="button" onClick={onClick} className="px-3 py-2 rounded text-sm font-medium transition-colors"
        style={{ background: selected ? CAT_ACCENT : "#2C2C2E", color: selected ? "#1C1C1E" : "#d1d5db", border: `1px solid ${selected ? CAT_ACCENT : "#374151"}` }}>
        {label}
      </button>
    )
  }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-48px)] bg-[#1C1C1E] text-white p-6">
      {/* Header */}
      <div className="flex items-end gap-3 mb-4 pb-3 border-b border-gray-800">
        <div>
          <p className="text-white font-bold text-base tracking-wide">CATALOGUING</p>
          <p className="text-xs mt-0.5 tracking-widest uppercase" style={{ color: CAT_ACCENT }}>Vectis</p>
        </div>
        <div className="grid grid-cols-2 gap-3 flex-1 max-w-lg">
          <div>
            ...
            <input value={auctionNo} onChange={e => setAuctionNo(e.target.value)} placeholder="e.g. 456" className={inpFocus} />
          </div>
          <div>
            <label className="text-xs text-gray-500 uppercase tracking-wider block mb-1">Auction Name</label>
            <input value={auctionName} onChange={e => setAuctionName(e.target.value)} placeholder="e.g. July 2025 Diecast" className={inpFocus} />
          </div>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0 pb-0.5">
          ...
          {lots.length > 0 && (
            <button onClick={exportExcel} className="px-3 py-1.5 bg-[#2C2C2E] border text-gray-300 text-xs rounded transition-colors" style={{ borderColor: CAT_ACCENT + "66" }}>
              ⬇ Export
            </button>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {step === 2 && (
          <div className="max-w-lg space-y-4">
            ...
            <button type="button" onClick={nextBarcodeNumber} className="px-4 py-2 text-sm rounded transition-colors"
        ...
        {step === 3 && (
          <div className="max-w-lg">
            ...
            <textarea value={keyPoints} onChange={e => setKeyPoints(e.target.value)} rows={6} placeholder="Describe any key points about this lot…" className={`${inpFocus} resize-none`} autoFocus />
          </div>
        )}
          ...
          <div className="max-w-lg space-y-5">
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button type="button" onClick={() => setPinnedMain(mainCat)} className="text-xs px-2 py-0.5 rounded transition-colors"
              ...
              <Autocomplete value={mainCat} onChange={v => { setMainCat(v); if (!CATEGORY_MAP[v]) setSubCat("") }} options={mainCatList} placeholder="Select main category…" accentColor={CAT_ACCENT} />
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs text-gray-500 uppercase tracking-wider">Sub Category</label>
                <button type="button" onClick={() => setPinnedSub(subCat)} className="text-xs px-2 py-0.5 rounded transition-colors"
              ...
              <Autocomplete value={subCat} onChange={setSubCat} options={subCats} placeholder={mainCat ? "Select sub-category…" : "Select main category first…"} accentColor={CAT_ACCENT} />
            </div>
            <div>
              <label className="text-xs text-gray-500 uppercase tracking-wider block mb-1">Brand</label>
              <Autocomplete value={brand} onChange={setBrand} options={BRANDS_LIST} placeholder="Search brand…" accentColor={CAT_ACCENT} />
            </div>
          </div>
        )}
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {QUICK_RANGES.map(([lo, hi], i) => (
                    <button key={i} type="button" onClick={() => { setEstLow(String(lo)); setEstHigh(String(hi)) }}
                      ...
                      style={{ background: estLow === String(lo) && estHigh === String(hi) ? CAT_ACCENT : "#1C1C1E", color: estLow === String(lo) && estHigh === String(hi) ? "#1C1C1E" : "#d1d5db", border: `1px solid ${estLow === String(lo) && estHigh === String(hi) ? CAT_ACCENT : "#374151"}` }}>
                      £{lo}–£{hi}
                    </button>
                  ))}
                </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {PARCEL_OPTIONS.map(opt => (
                  <button key={opt} type="button" onClick={() => setParcel(v => v === opt ? "" : opt)} className="px-4 py-2 rounded text-sm font-medium transition-colors"
                    style={{ background: parcel === opt ? CAT_ACCENT : "#2C2C2E", color: parcel === opt ? "#1C1C1E" : "#d1d5db", border: `1px solid ${parcel === opt ? CAT_ACCENT : "#374151"}` }}>
                    {opt}
                  </button>
                ))}
              </div>
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-800">
        <button onClick={goBack} disabled={step === 1} className="px-5 py-2 bg-[#2C2C2E] border border-gray-700 text-gray-300 text-sm rounded transition-colors disabled:opacity-30 hover:border-gray-500">← Back</button>
        <span className="text-xs text-gray-600">{step} / 7</span>
        {step < 7 ? (
          <button onClick={goNext} className="px-5 py-2 text-sm font-semibold rounded transition-colors" style={{ background: CAT_ACCENT, color: "#1C1C1E" }}>Next →</button>
        ) : (
          <button onClick={saveLot} className="px-5 py-2 text-sm font-semibold rounded transition-colors" style={{ background: CAT_ACCENT, color: "#1C1C1E" }}>Save Lot ✓</button>
        )}
      </div>
    </div>
  )
