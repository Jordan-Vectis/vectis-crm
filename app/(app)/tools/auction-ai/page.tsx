...
2) One estimate line in the form: Estimate: £X–£Y`,
  ...
  "Vectis Strict: Comics & Toys": `You create concise, accurate, professional auction catalogue entries for collectible comic books and toys for Vectis Auctions. Accuracy is prioritised. Condition: Mint, Near Mint, Excellent, Good, Fair, Poor. Plus may be used if an item is better than its classification suggests.
...
type Tab = "chat" | "batch" | "barcode" | "copier" | "catalogue"
...
function BatchTab() {
  ...
  async function run() {
    ...
    setDone(lotNames.length); setCurrent(""); setLoading(false)
  }
  ...
  const total = Object.keys(lots).length
  ...
  return (
    <div>
      ...
      <div onClick={() => ref.current?.click()}
        ...
        <p className="text-gray-300 text-sm font-medium mb-1">Select lot folder</p>
        <p className="text-gray-600 text-xs">Each sub-folder = one lot (up to 24 images each)</p>
        <input ref={ref} type="file" multiple className="hidden" {...({ webkitdirectory: "" } as any)} onChange={onFiles} />
      </div>

      {total > 0 && (
        <p className="text-sm text-gray-400 mb-4">
          <span className="text-[#C8A96E] font-semibold">{total}</span> lots · {Object.values(lots).reduce((s, f) => s + f.length, 0)} images
        </p>
      )}
      ...
      <button onClick={run} disabled={loading || !total}
        ...
        {loading ? `Processing ${done}/${total}…` : "Start Batch"}
      </button>
              ...
              <tbody className="divide-y divide-gray-800">
                {results.map((r, i) => (
                  <tr key={i} className="hover:bg-[#141416]">
                    <td className="px-4 py-2 text-gray-300 whitespace-nowrap">{r.lot}</td>
                    <td className="px-4 py-2 text-gray-500 text-xs max-w-xs truncate">{r.description || r.error}</td>
                    <td className="px-4 py-2 text-[#C8A96E] text-xs whitespace-nowrap">{r.estimate}</td>
                    <td className="px-4 py-2">
                      <span className={`text-xs font-bold ${r.status === "OK" ? "text-green-400" : "text-red-400"}`}>{r.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
...
type CatLot = {
  auctionNo: string; auctionName: string
  vendor: string; tote: string; receipt: string
  barcode: string; keyPoints: string
  mainCat: string; subCat: string; brand: string
  estLow: string; estHigh: string
  condition: string; parcel: string
  timestamp: string
}
...
function CataloguingTab() {
  ...
  const [lots,       setLots]       = useState<CatLot[]>([])
  ...
  function nextBarcodeNumber() {
    ...
    if (!m) return
    const num = m[1]
    setBarcode(src.slice(0, m.index) + String(parseInt(num) + 1).padStart(num.length, "0"))
  }
  ...
  function saveLot() {
    ...
    const condArr = [cond1, cond2].filter(Boolean)
      .sort((a, b) => CONDITIONS.indexOf(a) - CONDITIONS.indexOf(b))
    const condition = condArr.join(" to ")

    const lot: CatLot = {
      auctionNo, auctionName,
      vendor, tote, receipt,
      barcode, keyPoints,
      mainCat, subCat, brand,
      estLow:  estLow.replace(/[£,]/g,"").trim(),
      estHigh: estHigh.replace(/[£,]/g,"").trim(),
      condition, parcel,
      timestamp: new Date().toLocaleString(),
    }

    const newLots = [...lots, lot]
    setLots(newLots)
    setLastBarcode(barcode)
    setSaveStatus(`✓ Lot #${newLots.length} saved · ${vendor} / ${tote} / ${barcode.slice(0,40)}${barcode.length > 40 ? "…" : ""}`)

    // Reset per-lot fields, restore pinned
    setBarcode(""); setKeyPoints("")
    setMainCat(pinnedMain); setSubCat(pinnedSub)
    setBrand("")
    setEstLow(""); setEstHigh("")
    setCond1(""); setCond2("")
    setParcel("")
    setStep(1)
  }
  ...
  function exportExcel() {
    const headers = [
      "Auction No","Auction Name","Vendor","Tote","Receipt No",
      "Internal Barcode","Key Points","Main Category","Sub Category","Brand",
      "Estimate Low","Estimate High","Condition","Parcel Size","Timestamp",
    ]
    const rows = lots.map(l => [
      l.auctionNo, l.auctionName, l.vendor, l.tote, l.receipt,
      l.barcode, l.keyPoints, l.mainCat, l.subCat, l.brand,
      l.estLow, l.estHigh, l.condition, l.parcel, l.timestamp,
    ])
    const ws = XLSX.utils.aoa_to_sheet([headers, ...rows])
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, "Catalogue")
    XLSX.writeFile(wb, `catalogue_${auctionNo || "lots"}.xlsx`)
  }

  // Shared input style
  const inp = "w-full bg-[#2C2C2E] border border-gray-700 rounded px-3 py-2 text-sm text-gray-200 focus:outline-none"
  const inpFocus = `${inp} focus:border-[${CAT_ACCENT}]`
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header: Auction info + lot count */}
      <div className="flex items-end gap-3 mb-4 pb-3 border-b border-gray-800">
        <div className="grid grid-cols-2 gap-3 flex-1">
          <div>
            <label className="text-xs text-gray-500 uppercase tracking-wider block mb-1">Auction No</label>
            <input value={auctionNo} onChange={e => setAuctionNo(e.target.value)}
              placeholder="e.g. 456"
              className={inpFocus} />
          </div>
          <div>
            <label className="text-xs text-gray-500 uppercase tracking-wider block mb-1">Auction Name</label>
            <input value={auctionName} onChange={e => setAuctionName(e.target.value)}
              placeholder="e.g. July 2025 Diecast"
              className={inpFocus} />
          </div>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0 pb-0.5">
          <span className="text-green-400 text-sm font-bold whitespace-nowrap">{lots.length} lot{lots.length !== 1 ? "s" : ""}</span>
          {lots.length > 0 && (
            <button onClick={exportExcel}
              className="px-3 py-1.5 bg-[#2C2C2E] border border-gray-700 text-gray-300 text-xs rounded transition-colors"
              style={{ borderColor: CAT_ACCENT + "66" }}>
              ⬇ Export
            </button>
          )}
        </div>
      ...
      <div className="flex mb-5 border-b border-gray-800">
        {STEP_LABELS.map((label, i) => (
          <div key={i} className="flex-1 min-w-0 pb-2 text-center border-b-2 transition-colors"
            style={{
              borderColor: i + 1 === step ? CAT_ACCENT : i + 1 < step ? "#22c55e" : "#374151",
            }}>
      ...
      <div className="flex-1 overflow-y-auto">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input value={vendor} onChange={e => setVendor(e.target.value)}
                  className={`flex-1 ${inpFocus}`} placeholder="e.g. V12345" autoFocus />
                {vendor && <button type="button" onClick={() => setVendor("")}
                  className="px-3 py-2 bg-[#2C2C2E] border border-gray-700 text-gray-500 text-xs rounded hover:border-red-500 hover:text-red-400">✕</button>}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                <input value={tote} onChange={e => setTote(e.target.value)}
                  className={`flex-1 ${inpFocus}`} placeholder="e.g. T001" />
                {tote && <button type="button" onClick={() => setTote("")}
                  className="px-3 py-2 bg-[#2C2C2E] border border-gray-700 text-gray-500 text-xs rounded hover:border-red-500 hover:text-red-400">✕</button>}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                <input value={receipt} onChange={e => setReceipt(e.target.value)}
                  className={`flex-1 ${inpFocus}`} placeholder="e.g. R9876" />
                {receipt && <button type="button" onClick={() => setReceipt("")}
                  className="px-3 py-2 bg-[#2C2C2E] border border-gray-700 text-gray-500 text-xs rounded hover:border-red-500 hover:text-red-400">✕</button>}
              </div>
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="max-w-lg">
            ...
            <textarea value={keyPoints} onChange={e => setKeyPoints(e.target.value)}
              rows={6} placeholder="Describe any key points about this lot…"
          ...
          <div className="max-w-lg space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button type="button"
                  onClick={() => setPinnedMain(mainCat)}
                  className="text-xs px-2 py-0.5 rounded transition-colors"
                  style={{ color: pinnedMain === mainCat && mainCat ? CAT_ACCENT : "#6b7280",
                    border: `1px solid ${pinnedMain === mainCat && mainCat ? CAT_ACCENT + "66" : "#374151"}` }}>
                  {pinnedMain === mainCat && mainCat ? "📌 Pinned" : "Pin"}
                </button>
              </div>
              <Autocomplete
                value={mainCat}
                onChange={v => { setMainCat(v); if (!CATEGORY_MAP[v]) setSubCat("") }}
                options={mainCatList}
                placeholder="Select main category…"
                accentColor={CAT_ACCENT}
              />
            </div>
            {/* Sub Category */}
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button type="button"
                  onClick={() => setPinnedSub(subCat)}
                  className="text-xs px-2 py-0.5 rounded transition-colors"
                  style={{ color: pinnedSub === subCat && subCat ? CAT_ACCENT : "#6b7280",
                    border: `1px solid ${pinnedSub === subCat && subCat ? CAT_ACCENT + "66" : "#374151"}` }}>
              ...
              <Autocomplete
                value={subCat}
                onChange={setSubCat}
                options={subCats.length ? subCats : (mainCat ? [] : [])}
            ...
            <div className="flex gap-6">
              ...
              <div className="flex-1 space-y-3">
                <div>
                  ...
                  <input value={estLow} onChange={e => setEstLow(e.target.value)}
                    className={inpFocus} placeholder="e.g. 40" autoFocus />
                </div>
                <div>
                  ...
                  <input value={estHigh} onChange={e => setEstHigh(e.target.value)}
                    className={inpFocus} placeholder="e.g. 60" />
                </div>
              </div>
              {/* Quick select */}
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {QUICK_RANGES.map(([lo, hi], i) => (
                    <button key={i} type="button"
                      onClick={() => { setEstLow(String(lo)); setEstHigh(String(hi)) }}
                      className="px-2 py-1.5 text-xs rounded transition-colors hover:text-black whitespace-nowrap"
                      style={{
        ...
        {step === 6 && (
          <div className="max-w-lg space-y-5">
            <div>
              <label className="text-xs text-gray-500 uppercase tracking-wider block mb-2">Condition <span className="text-red-500">*</span></label>
              <div className="flex flex-wrap gap-2">
                {CONDITIONS.map(c => (
                  <CondBtn key={c} label={c} selected={cond1 === c}
                    onClick={() => setCond1(v => v === c ? "" : c)} />
                ))}
              </div>
            </div>
            <div>
              <label className="text-xs text-gray-500 uppercase tracking-wider block mb-1">
                Condition To <span className="text-gray-600">(optional — click again to deselect)</span>
              </label>
              <div className="flex flex-wrap gap-2">
                {CONDITIONS.map(c => (
                  <CondBtn key={c} label={c} selected={cond2 === c}
                    onClick={() => setCond2(v => v === c ? "" : c)} />
                ))}
              </div>
            </div>
          </div>
        ...
        {step === 7 && (
          <div className="max-w-lg space-y-4">
            ...
            {saveStatus && (
              <p className="text-green-400 text-sm font-medium">{saveStatus}</p>
            )}
          </div>
        )}
      </div>
...
const TABS: { id: Tab; label: string; icon: string; accent?: string }[] = [
  ...
  { id: "catalogue", label: "Cataloguing",        icon: "📂", accent: CAT_ACCENT },
]
