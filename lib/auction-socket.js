...
function setupAuctionSocket(io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('bid:commission', ({ lotId, maxAmount, bidderId, bidderName }) => {
      if (!activeAuction) {
      ...
      // Replace any existing auto bid from this bidder on this lot
      lot.autoBids = lot.autoBids.filter(a => a.bidderId !== bidderId)
      ...
      // Refresh auctioneer if this is the current lot
      io.emit('auction:state', getPublicState())
    })
