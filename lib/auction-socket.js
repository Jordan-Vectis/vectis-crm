...
async function loadLotsFromDB(auctionId) {
  const { rows } = await pool.query(`
    ...
    FROM "CatalogueLot"
    WHERE "auctionId" = $1
    ORDER BY "lotNumber"::int NULLS LAST, "lotNumber"
  `, [auctionId])
...
async function persistHammer(lotId, hammerPrice) {
  await pool.query(
    `UPDATE "CatalogueLot" SET "hammerPrice" = $1, status = 'SOLD', "updatedAt" = NOW() WHERE id = $2`,
    [hammerPrice, lotId]
  )
}
...
function setupAuctionSocket(io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('clerk:selectAuction', async ({ auctionId }) => {
      ...
      try {
        ...
        activeAuction = {
          ...
          status: 'PENDING', currentLotIndex: 0, fairWarning: false,
        }
        activeLots = lots

        // Check for existing live state in DB
        const { rows: liveRows } = await pool.query(
          `SELECT * FROM "LiveAuction" WHERE "auctionId" = $1`, [auctionId]
        )
        if (liveRows[0]) {
          activeAuction.status = liveRows[0].status
          activeAuction.currentLotIndex = liveRows[0].currentLotIndex
        }
    ...
    socket.on('clerk:hammer', async () => {
      ...
      io.emit('lot:hammer', { lotNumber: lot.lotNumber, hammerPrice: lot.hammerPrice })
      io.emit('auction:state', getPublicState())
