...
// Every database change the Run Migrations button (and the admin banner) applies.
//
// Moved here from app/api/admin/run-migrations/route.ts on 2026-09-10 so the Status Centre can
// compare the SAME fingerprint — a Next route file can only export its handlers, and a copy
// would drift. ⚠ RULES.md: a Prisma migration FILE alone does nothing on Railway — every new
// table or column must ALSO be added to this array (CREATE ... IF NOT EXISTS / ADD COLUMN IF NOT EXISTS).
export const MIGRATIONS = [
  ...
  `ALTER TABLE "CatalogueLotEvent" ADD COLUMN IF NOT EXISTS "action" TEXT NOT NULL DEFAULT 'updated'`,
 ...
 ON CONFLICT ("categoryId","name") DO NOTHING`,
  ...
  // Facilities → First Aid. Everything except AccidentReport is shown on the PUBLIC page.
  ...
  // Who catalogued a BC line, for the ~50k rows where EVA_CataloguedBy is blank.
  `ALTER TABLE "WarehouseItem" ADD COLUMN IF NOT EXISTS "cataloguedByUser" TEXT`,
  `ALTER TABLE "WarehouseItem" ADD COLUMN IF NOT EXISTS "bcCreatedBy" TEXT`,
  ...
  `CREATE TABLE IF NOT EXISTS "CataloguerLotStart" (
     ...
     CONSTRAINT "CataloguerLotStart_pkey" PRIMARY KEY ("userId")
   )`,
  `DO $$ BEGIN
    ALTER TABLE "CataloguerLotStart" ADD CONSTRAINT "CataloguerLotStart_userId_fkey"
      FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;
  EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
  ...
  // BC's own truth about which receipt a tote is on. EVA_TOT_ReceiptTote is keyed on
  // (Receipt No., Line No.), so one tote number can sit on several receipts — WarehouseTote is
  // unique on toteNo and keeps only one. Separate table on purpose: every existing check reads
  // WarehouseTote and must keep reporting exactly what it reports today.
  ...
  // 🚦 Status Centre (/admin/status) + the admin bell — current state per service, every check
  // result (uptime history, pruned after 30 days), and the bell's notifications. See lib/status/.
  ...
  // 📝 Feedback surveys (Admin → Feedback) — written-answer questions sent to chosen roles/people as a
  // popup; one response row per person per survey (LATER = put off, SUBMITTED = done). See lib/feedback.ts.
...
export const MIGRATIONS_HASH = createHash("sha256").update(MIGRATIONS.join("\n")).digest("hex")
